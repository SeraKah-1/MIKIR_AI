...
import { Key, Save, Trash2, ShieldCheck, ExternalLink, Zap, Cpu, Database, HardDrive, Server, Layers, HelpCircle, CheckCircle2, AlertTriangle, Wifi, Copy, Terminal, Palette, LogOut, CreditCard, ShieldAlert, ArrowRight, Wrench } from 'lucide-react';
...
const SQL_CHEAT_SHEET = `-- SQL SETUP UNTUK MIKIR ( •_•)
-- Copy semua kode di bawah ini dan jalankan di SQL Editor Supabase
...
-- 2. Tabel Notes (Opsional, jika mau integrasi catatan)
...
create policy "Public Access Quizzes" on public.generated_quizzes for all to anon using (true) with check (true);
create policy "Public Access Notes" on public.notes for all to anon using (true) with check (true);`;
...
export const SettingsScreen: React.FC = () => {
  ...
  const handleLogout = () => {
     if (confirm("Logout dan cabut akses Keycard? Aplikasi akan dikunci kembali.")) {
        logoutKeycard();
        // Force full reload to ensure LoginGate catches the cleared state
  ...
  const handleTestSupabase = async () => {
    ...
    try {
      const tempClient = getSupabaseClient(supabaseUrl, supabaseKey);
      const { error } = await tempClient.from('generated_quizzes').select('id').limit(1);
      if (error && error.message.includes('relation') && error.message.includes('does not exist')) {
           alert("Koneksi OK, tapi Tabel belum ada. Jalankan SQL Schema!"); setConnectionStatus('success'); setShowGuide(true);
      } else if(error) { throw error; } 
      else { setConnectionStatus('success'); notifySupabaseSuccess(); }
    } catch (err: any) { setConnectionStatus('error'); notifySupabaseError(); } 
  ...
  const handleCopySql = () => { navigator.clipboard.writeText(SQL_CHEAT_SHEET); setCopiedSql(true); setTimeout(() => setCopiedSql(false), 2000); };
  ...
  // Common styles for inputs to ensure visibility across themes
    ...
    <div className="max-w-2xl mx-auto pt-8 pb-32 px-4 text-theme-text relative">
            ...
            <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                ...
                >
                    ...
                    <input 
                      ...
                      className="w-full border border-slate-300 rounded-xl px-4 py-3 mb-4 focus:ring-2 focus:ring-indigo-500 outline-none text-slate-800 bg-white"
                    />
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white mb-8 shadow-lg relative overflow-hidden">
           <div className="relative z-10 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                 ...
                 <div>
                    <h3 className="font-bold text-lg">{sessionMetadata.owner}</h3>
                    <p className="text-white/60 text-xs">Logged in via Keycard</p>
                 </div>
              </div>
              <button 
                onClick={handleLogout} 
                className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold flex items-center transition-colors cursor-pointer"
              >
                 <LogOut size={14} className="mr-2" /> Eject Card
              </button>
           </div>
      ...
      <div className="flex space-x-2 md:space-x-4 mb-8 justify-center overflow-x-auto pb-2 scrollbar-hide">
         {['ai', 'storage', 'appearance', 'features'].map(tab => (
           <button 
             key={tab} 
             onClick={() => setStorageTab(tab as any)} 
             className={`px-6 py-2 rounded-full font-medium transition-all whitespace-nowrap capitalize ${storageTab === tab ? tabActive : tabInactive}`}
            >
             {tab}
           </button>
         ))}
        ...
        {storageTab === 'ai' && (
          <>
            <div className="flex items-center space-x-3 mb-6 select-none">
                ...
                <div><h2 className="text-2xl font-bold">API Key</h2><p className="text-sm opacity-70">Akses Gemini & Groq.</p></div>
            </div>
            
            {sessionMetadata ? (
               <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-600 text-sm flex items-center">
                  <ShieldCheck size={18} className="mr-2" /> API Keys dikelola oleh Keycard. Mode Read-Only.
               </div>
            ) : (
                <>
                {/* PROVIDER TABS UPDATED FOR VISIBILITY */}
                <div className="flex bg-theme-glass p-1 rounded-xl mb-6 border border-theme-border">
                <button 
                   onClick={() => setActiveTab('gemini')} 
                   className={`flex-1 py-2 rounded-lg flex items-center justify-center text-sm font-medium transition-all ${activeTab === 'gemini' ? 'bg-theme-primary text-white shadow-md' : 'text-theme-muted hover:text-theme-text hover:bg-theme-bg/50'}`}
                >
                   <Zap size={16} className="mr-2" /> Gemini
                </button>
                <button 
                   onClick={() => setActiveTab('groq')} 
                   className={`flex-1 py-2 rounded-lg flex items-center justify-center text-sm font-medium transition-all ${activeTab === 'groq' ? 'bg-orange-500 text-white shadow-md' : 'text-theme-muted hover:text-orange-500 hover:bg-theme-bg/50'}`}
                >
                ...
                <div>
                    ...
                    <input 
                      type="password" 
                      value={activeTab === 'gemini' ? geminiKey : groqKey} 
                      onChange={(e) => activeTab === 'gemini' ? setGeminiKey(e.target.value) : setGroqKey(e.target.value)} 
            ...
            {storageProvider === 'supabase' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4 mb-6">
                ...
                <div className="mt-8 border-t border-theme-border pt-6">
                  <button onClick={() => setShowGuide(!showGuide)} className="flex items-center w-full text-sm font-bold text-theme-primary hover:underline"><HelpCircle size={16} className="mr-2" /> {showGuide ? "Tutup Panduan" : "Panduan Setup & SQL"}</button>
                  <AnimatePresence>
                     {showGuide && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="overflow-hidden">
                           <div className="bg-theme-glass border border-theme-border rounded-xl p-5 mt-4 text-sm opacity-80 space-y-4">
                              <p>Paste kode ini di Supabase SQL Editor:</p>
                              <div className="relative group">
                                 <pre className="bg-slate-900 text-slate-300 p-4 rounded-xl text-xs overflow-x-auto font-mono border border-slate-700">{SQL_CHEAT_SHEET}</pre>
      ...
      {/* NEW: EXPLICIT ADMIN ACCESS BUTTON AT BOTTOM */}
      <div className="mt-12 text-center">
         <button 
           onClick={() => setShowAdminAuth(true)} 
           className="inline-flex items-center px-4 py-2 rounded-full text-xs font-bold text-theme-muted opacity-40 hover:opacity-100 hover:bg-theme-glass transition-all"
         >
            <Wrench size={12} className="mr-2" /> Admin Console
         </button>
      </div>
    </div>
  );
